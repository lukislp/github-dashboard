...
- **Totals**: repositories, open pull requests, open issues, failed runs, running runs.
...
| `GITHUB_SCOPES` | no | `repo read:org` | `repo` is needed for private repositories and their runs |
...
Dependencies point inwards only. Repositories with their open PR/issue counts come from one
paginated GraphQL query; workflow runs come from the REST API, fetched concurrently with a
semaphore. Archived repositories are not queried for runs.
